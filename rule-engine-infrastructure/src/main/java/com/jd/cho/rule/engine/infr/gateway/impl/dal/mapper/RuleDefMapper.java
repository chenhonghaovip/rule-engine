...
import static com.jd.cho.rule.engine.infr.gateway.impl.dal.mapper.RuleDefDynamicSqlSupport.*;
...
public interface RuleDefMapper {
    ...
    BasicColumn[] selectList = BasicColumn.columnList(id, ruleCode, ruleName, version, latest, priority, remark, status, yn, createTime, modifyTime, creator, modifier, tenant, ruleCondition, ruleAction);
    ...
    default int insert(RuleDefDO record) {
        return MyBatis3Utils.insert(this::insert, record, ruleDef, c ->
                c.map(id).toProperty("id")
                        .map(ruleCode).toProperty("ruleCode")
    ...
    default int insertSelective(RuleDefDO record) {
        return MyBatis3Utils.insert(this::insert, record, ruleDef, c ->
                c.map(id).toPropertyWhenPresent("id", record::getId)
                        .map(ruleCode).toPropertyWhenPresent("ruleCode", record::getRuleCode)
    ...
    static UpdateDSL<UpdateModel> updateAllColumns(RuleDefDO record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalTo(record::getId)
                .set(ruleCode).equalTo(record::getRuleCode)
    ...
    static UpdateDSL<UpdateModel> updateSelectiveColumns(RuleDefDO record, UpdateDSL<UpdateModel> dsl) {
        return dsl.set(id).equalToWhenPresent(record::getId)
                .set(ruleCode).equalToWhenPresent(record::getRuleCode)
    ...
    default int updateByPrimaryKey(RuleDefDO record) {
        return update(c ->
                c.set(ruleCode).equalTo(record::getRuleCode)
                        .set(ruleName).equalTo(record::getRuleName)
                        .set(version).equalTo(record::getVersion)
                        .set(latest).equalTo(record::getLatest)
                        .set(priority).equalTo(record::getPriority)
    ...
    default int updateByPrimaryKeySelective(RuleDefDO record) {
        return update(c ->
                c.set(ruleCode).equalToWhenPresent(record::getRuleCode)
                        .set(ruleName).equalToWhenPresent(record::getRuleName)
                        .set(version).equalToWhenPresent(record::getVersion)
                        .set(latest).equalToWhenPresent(record::getLatest)
                        .set(priority).equalToWhenPresent(record::getPriority)
